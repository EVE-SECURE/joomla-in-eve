...
class EvechartrackingModelEvechartracking  extends JModel {
	...
	function getCorps() {
		...
		$q->addWhere('co.corporationID IN ('.$corps.')');
	...
	function getMemberCount($ID, $layout) {
		...
		$q->addQuery('COUNT(ch.id)');
		return $q->loadResult();
		
	}
	
	function getColumns() {
		$result =  array('race', 'gender', 'bloodLine', 'balance', 'startDateTime', 'title', 'baseName', 
