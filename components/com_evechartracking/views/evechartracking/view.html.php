...
class EvechartrackingViewEvechartracking extends JView {
	function display() {
		...
		if ($layout == 'corp') {
			...
			$userID = $mainframe->getUserStateFromRequest( $context.'userID', 'userID', 0, 'int' );
			$defaultColums = array();
			if (JRequest::getInt('reset') && is_null(JRequest::getVar('selectedColumns'))) {
				$mainframe->setUserState($context.'selectedColumns', array());
			}
			...
			$html_users = JHTML::_('select.genericlist', $users, 'userID', array('onchange'=>'this.form.submit()'), 'id', 'name', $userID);
	...
	function getMemberColumn(&$member, $column) {
		switch($column) {
			...
			case 'userID':
				...
				return sprintf('<a href="%s">%s</a>', JRoute::_('index.php?option=com_evechartracking&view=evechartracking&layout=user&userID='.$member->userID), $member->userName);				
				case 'startDateTime':
			case 'startDateTime':
			case 'logonDateTime':
			case 'logoffDateTime':
