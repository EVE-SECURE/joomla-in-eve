...
?>
<form
	action="<?php echo JRoute::_('index.php?option=com_eve&view=characters'); ?>"
	method="post" name="adminForm">
<fieldset if="filter-bar">
<div class="filter-search fltlft"><?php echo JHTML::_('filter.search', $this->state->get('filter.search')); ?>
</div>
<div class="filter-select fltrt"><?php echo JHTML::_('filter.membersof', $this->state->get('filter.membersof')); ?>
</div>
</fieldset>
<table class="adminlist" cellspacing="1">
	...
	<?php foreach ($this->items as $i => $item) : ?>
		<tr class="row<?php echo $i % 2; ?>">
			...
			<td><?php echo JHtml::_('grid.checkedout', $item, $i, 'characterID'); ?>
			</td>
			<td><?php if (JTable::isCheckedOut($userId, $item->checked_out)) : ?>
			<?php echo $item->name; ?> (<?php echo $item->characterID; ?>) <?php else : ?>
			<span class="editlinktip hasTip"
				title="<?php echo JText::_('Edit item');?>::<?php echo $item->name; ?>">
			<a
				href="<?php echo JRoute::_('index.php?option=com_eve&task=character.edit&characterID='.$item->characterID); ?>">
				<?php echo $item->name; ?></a></span> (<?php echo $item->characterID; ?>)
				<?php endif; ?></td>
			<td align="center"><?php echo $item->userName; ?></td>
			<td><?php echo $item->corporationName; ?></td>
			<td><?php echo $item->allianceName; ?></td>
			<td><?php if (!is_null($item->status)): ?> <span
				class="apiStatus apiStatus-<?php echo $item->status; ?>"><?php echo $item->status; ?></span>
				<?php endif; ?></td>
		</tr>
		<?php endforeach; ?>
